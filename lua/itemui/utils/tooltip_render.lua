...
--- Task 7.5: Render side-by-side stat blocks with deltas (left = hovered, right = equipped).
--- colW = column width for the two columns.
function M.renderComparisonStatBlocks(leftItem, rightItem, ctx, api, colW)
    if not leftItem then return end
    colW = colW or (api.tooltip_layout and api.tooltip_layout.COMPARISON_TOOLTIP_COL_WIDTH) or 500
    api = api or {}
    local formatSize = api.formatSize or function(item) return item and item.size and tostring(item.size) or nil end
    local attrLine = api.attrLine or function(b, h, label)
        b, h = b or 0, h or 0
        if b == 0 and h == 0 then return nil end
        if h > 0 then return string.format("%s: %d+%d", label, b, h) end
        return string.format("%s: %d", label, b)
    ...
    for _, item in ipairs({ leftItem, rightItem }) do
        if item then
            local _ = item.augSlots
            _ = item.wornSlots
            _ = item.ac
    ...
    local function num(v) local n = tonumber(v); return (n and n ~= 0) and n or nil end
    local function row(label, leftVal, rightVal, leftStr, rightStr)
        leftStr = leftStr or (leftVal ~= nil and tostring(leftVal)) or "—"
        rightStr = rightStr or (rightVal ~= nil and rightItem and tostring(rightVal)) or "—"
        if not rightItem then rightStr = "—" end
        local leftNum = num(leftVal)
        local rightNum = rightItem and num(rightVal) or nil
        local delta = nil
        if leftNum ~= nil and rightNum ~= nil then delta = leftNum - rightNum end
        return { label = label, leftStr = leftStr, rightStr = rightStr, delta = delta }
    end
    local rows = {}
    -- Item info (same order as normal tooltip where applicable)
    if formatSize(leftItem) or (rightItem and formatSize(rightItem)) then
        rows[#rows + 1] = row("Size", nil, nil, formatSize(leftItem), rightItem and formatSize(rightItem) or nil)
    end
    local wL = (leftItem.weight and leftItem.weight ~= 0) and (leftItem.weight >= 10 and string.format("%.1f", leftItem.weight / 10) or tostring(leftItem.weight)) or nil
    local wR = rightItem and (rightItem.weight and rightItem.weight ~= 0) and (rightItem.weight >= 10 and string.format("%.1f", rightItem.weight / 10) or tostring(rightItem.weight)) or nil
    if wL or wR then rows[#rows + 1] = row("Weight", leftItem.weight, rightItem and rightItem.weight or nil, wL, wR) end
    if (leftItem.requiredLevel and leftItem.requiredLevel ~= 0) or (rightItem and rightItem.requiredLevel and rightItem.requiredLevel ~= 0) then
        rows[#rows + 1] = row("Req Level", leftItem.requiredLevel, rightItem and rightItem.requiredLevel or nil)
    end
    if (leftItem.recommendedLevel and leftItem.recommendedLevel ~= 0) or (rightItem and rightItem.recommendedLevel and rightItem.recommendedLevel ~= 0) then
        rows[#rows + 1] = row("Rec Level", leftItem.recommendedLevel, rightItem and rightItem.recommendedLevel or nil)
    end
    if (leftItem.type and leftItem.type ~= "") or (rightItem and rightItem.type and rightItem.type ~= "") then
        rows[#rows + 1] = row("Skill", nil, nil, leftItem.type or "—", rightItem and (rightItem.type or "—") or "—")
    end
    if (leftItem.ac and leftItem.ac ~= 0) or (rightItem and rightItem.ac and rightItem.ac ~= 0) then
        rows[#rows + 1] = row("AC", leftItem.ac, rightItem and rightItem.ac or nil)
    end
    if (leftItem.hp and leftItem.hp ~= 0) or (rightItem and rightItem.hp and rightItem.hp ~= 0) then
        rows[#rows + 1] = row("HP", leftItem.hp, rightItem and rightItem.hp or nil)
    end
    if (leftItem.mana and leftItem.mana ~= 0) or (rightItem and rightItem.mana and rightItem.mana ~= 0) then
        rows[#rows + 1] = row("Mana", leftItem.mana, rightItem and rightItem.mana or nil)
    end
    if (leftItem.endurance and leftItem.endurance ~= 0) or (rightItem and rightItem.endurance and rightItem.endurance ~= 0) then
        rows[#rows + 1] = row("End", leftItem.endurance, rightItem and rightItem.endurance or nil)
    end
    if (leftItem.haste and leftItem.haste ~= 0) or (rightItem and rightItem.haste and rightItem.haste ~= 0) then
        rows[#rows + 1] = row("Haste", leftItem.haste, rightItem and rightItem.haste or nil, (leftItem.haste and leftItem.haste ~= 0) and (tostring(leftItem.haste) .. "%") or "—", rightItem and (rightItem.haste and rightItem.haste ~= 0) and (tostring(rightItem.haste) .. "%") or "—" or "—")
    end
    if (leftItem.purity and leftItem.purity ~= 0) or (rightItem and rightItem.purity and rightItem.purity ~= 0) then
        rows[#rows + 1] = row("Purity", leftItem.purity, rightItem and rightItem.purity or nil)
    end
    local isWeaponL = (leftItem.damage and leftItem.damage ~= 0) or (leftItem.itemDelay and leftItem.itemDelay ~= 0) or (leftItem.type and leftItem.type ~= "" and tostring(leftItem.type):lower():find("piercing"))
    local isWeaponR = rightItem and ((rightItem.damage and rightItem.damage ~= 0) or (rightItem.itemDelay and rightItem.itemDelay ~= 0))
    if isWeaponL or isWeaponR then
        rows[#rows + 1] = row("Base Dmg", leftItem.damage, rightItem and rightItem.damage or nil)
        rows[#rows + 1] = row("Delay", leftItem.itemDelay, rightItem and rightItem.itemDelay or nil)
        rows[#rows + 1] = row("Dmg Bon", leftItem.dmgBonus, rightItem and rightItem.dmgBonus or nil)
    end
    -- All Stats: primary
    local attrs = { { leftItem.str, leftItem.heroicSTR, rightItem and rightItem.str, rightItem and rightItem.heroicSTR, "Strength" },
        { leftItem.sta, leftItem.heroicSTA, rightItem and rightItem.sta, rightItem and rightItem.heroicSTA, "Stamina" },
        { leftItem.int, leftItem.heroicINT, rightItem and rightItem.int, rightItem and rightItem.heroicINT, "Intelligence" },
        { leftItem.wis, leftItem.heroicWIS, rightItem and rightItem.wis, rightItem and rightItem.heroicWIS, "Wisdom" },
        { leftItem.agi, leftItem.heroicAGI, rightItem and rightItem.agi, rightItem and rightItem.heroicAGI, "Agility" },
        { leftItem.dex, leftItem.heroicDEX, rightItem and rightItem.dex, rightItem and rightItem.heroicDEX, "Dexterity" },
        { leftItem.cha, leftItem.heroicCHA, rightItem and rightItem.cha, rightItem and rightItem.heroicCHA, "Charisma" },
    }
    for _, a in ipairs(attrs) do
        local lLine = attrLine(a[1], a[2], a[5])
        local rLine = rightItem and attrLine(a[3], a[4], a[5]) or nil
        if lLine or rLine then
            local lNum = (tonumber(a[1]) or 0) + (tonumber(a[2]) or 0)
            local rNum = rightItem and ((tonumber(a[3]) or 0) + (tonumber(a[4]) or 0)) or nil
            rows[#rows + 1] = row(a[5], lNum, rNum, lLine or "—", rLine or "—")
        end
    end
    -- Resists
    local function resistLine(b, h) b, h = b or 0, h or 0; if b == 0 and h == 0 then return nil end; if h > 0 then return b + h end; return b end
    local resists = { { leftItem.svMagic, leftItem.heroicSvMagic, rightItem and rightItem.svMagic, rightItem and rightItem.heroicSvMagic, "Magic" },
        { leftItem.svFire, leftItem.heroicSvFire, rightItem and rightItem.svFire, rightItem and rightItem.heroicSvFire, "Fire" },
        { leftItem.svCold, leftItem.heroicSvCold, rightItem and rightItem.svCold, rightItem and rightItem.heroicSvCold, "Cold" },
        { leftItem.svDisease, leftItem.heroicSvDisease, rightItem and rightItem.svDisease, rightItem and rightItem.heroicSvDisease, "Disease" },
        { leftItem.svPoison, leftItem.heroicSvPoison, rightItem and rightItem.svPoison, rightItem and rightItem.heroicSvPoison, "Poison" },
        { leftItem.svCorruption, leftItem.heroicSvCorruption, rightItem and rightItem.svCorruption, rightItem and rightItem.heroicSvCorruption, "Corruption" },
    }
    for _, r in ipairs(resists) do
        local lVal = resistLine(r[1], r[2])
        local rVal = rightItem and resistLine(r[3], r[4]) or nil
        if lVal and lVal ~= 0 or (rVal and rVal ~= 0) then
            local lStr = attrLine(r[1], r[2], r[5])
            local rStr = rightItem and attrLine(r[3], r[4], r[5]) or "—"
            rows[#rows + 1] = row(r[5], lVal, rVal, lStr or "—", rStr)
        end
    local combat = { "attack", "HP Regen", "hpRegen"; "manaRegen", "Mana Regen"; "enduranceRegen", "End Regen"; "accuracy", "Accuracy"; "strikeThrough", "Strike Thr"; "healAmount", "Heal Amount"; "spellDamage", "Spell Dmg"; "spellShield", "Spell Shield"; "shielding", "Shielding"; "avoidance", "Avoidance"; "stunResist", "Stun Resist"; }
    end
    -- Combat
    local combatList = {
        { "attack", "Attack" }, { "hpRegen", "HP Regen" }, { "manaRegen", "Mana Regen" }, { "enduranceRegen", "End Regen" },
        { "combatEffects", "Combat Eff" }, { "damageShield", "Dmg Shield" }, { "damageShieldMitigation", "Dmg Shld Mit" },
        { "accuracy", "Accuracy" }, { "strikeThrough", "Strike Thr" }, { "healAmount", "Heal Amount" }, { "spellDamage", "Spell Dmg" },
        { "spellShield", "Spell Shield" }, { "shielding", "Shielding" }, { "dotShielding", "DoT Shield" }, { "avoidance", "Avoidance" },
        { "stunResist", "Stun Resist" }, { "clairvoyance", "Clairvoyance" }, { "luck", "Luck" },
    }
    for _, c in ipairs(combatList) do
        local lVal = leftItem[c[1]]
        local rVal = rightItem and rightItem[c[1]] or nil
        if (lVal and tonumber(lVal) and tonumber(lVal) ~= 0) or (rVal and tonumber(rVal) and tonumber(rVal) ~= 0) then
            rows[#rows + 1] = row(c[2], lVal, rVal)
        end
    end
    -- Draw two columns with deltas on left
    ImGui.Columns(2, "##CompareStats", false)
    ImGui.SetColumnWidth(0, colW)
    ImGui.SetColumnWidth(1, colW)
    local green = ImVec4(0.35, 0.85, 0.35, 1.0)
    local red = ImVec4(0.9, 0.35, 0.35, 1.0)
    local grey = ImVec4(0.55, 0.55, 0.55, 1.0)
    for _, r in ipairs(rows) do
        local leftText = r.label .. ": " .. r.leftStr
        if r.delta ~= nil then
            if r.delta > 0 then
                leftText = leftText .. "  (" .. tostring("+" .. r.delta) .. ")"
                ImGui.TextColored(green, leftText)
            elseif r.delta < 0 then
                leftText = leftText .. "  (" .. tostring(r.delta) .. ")"
                ImGui.TextColored(red, leftText)
            else
                leftText = leftText .. "  (0)"
                ImGui.TextColored(grey, leftText)
            end
        else
            ImGui.Text(leftText)
        end
        ImGui.NextColumn()
        ImGui.Text(r.label .. ": " .. r.rightStr)
        ImGui.NextColumn()
    end
end
