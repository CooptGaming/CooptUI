...
local state = { exportPath = "", importPath = "", importPreview = nil, importConfirmOpen = false }

local backupService
local function getBackupService()
    if not backupService then backupService = require('itemui.services.backup_service') end
...
function ConfigAdvanced.render(ctx)
    ...
    if ImGui.CollapsingHeader("Backup & Restore", ImGuiTreeNodeFlags.None) then
        ...
        if ImGui.Button("Export##Backup", ImVec2(100, 0)) then
            local ok, result = bak.exportPackage(state.exportPath)
            if ok then setStatusMessage(string.format("Export complete. %d files written.", result or 0))
            else setStatusMessage("Export failed: " .. tostring(result or "unknown")) end
        end
        ImGui.Spacing()
        ImGui.SetNextItemWidth(320)
        ...
        if ImGui.Button("Import##Backup", ImVec2(100, 0)) then
            local preview, err = bak.previewImport(state.importPath)
            if err then setStatusMessage("Import preview failed: " .. tostring(err))
            else state.importPreview = preview; state.importConfirmOpen = true end
        end
        ...
        if ImGui.BeginPopupModal("Confirm Import##Backup", nil, ImGuiWindowFlags.AlwaysAutoResize) then
            ...
            if ImGui.Button("Confirm Import##BackupBtn", ImVec2(120, 0)) then
                local ok, result = bak.importPackage(state.importPath)
                if ok then setStatusMessage(string.format("Import complete. %d files restored.", result or 0))
                else setStatusMessage("Import failed: " .. tostring(result or "unknown")) end
                state.importConfirmOpen = false; state.importPreview = nil
        ...
        if bak.hasRestoreBackup and bak.hasRestoreBackup() then
            ImGui.Spacing()
            if ImGui.Button("Restore Previous (from .bak)##Backup", ImVec2(220, 0)) then
                local ok, result = bak.restoreFromBackup()
                if ok then setStatusMessage(string.format("Restore complete. %d files restored from backup.", result or 0))
                else setStatusMessage("Restore failed: " .. tostring(result or "unknown")) end
            end
            if ImGui.IsItemHovered() then ImGui.BeginTooltip(); ImGui.Text("Restore files from the backup created by the last Import."); ImGui.EndTooltip() end
        end
    end
end
