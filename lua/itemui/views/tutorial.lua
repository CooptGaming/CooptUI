...
local function renderInventoryOverviewOverlay(refs)
    ...
    ImGui.BulletText("Right-click menu: Inspect, Move to Bank, add to Keep/Sell lists, Item info, and more.")
...
local function renderRemainingCompanionsOverlay(refs)
    ...
    bulletWrapped("Full stat sheet for any item. Right-click an item anywhere and choose \"Item info\" to open it here.")
    ...
    -- The standalone Augments window used to have its own entry here. It folded into this
    -- window's second tab in phase 11 and its id is classicOnly, so with the bars up it can
    -- never be opened -- the wizard was teaching a window this mode cannot draw. Same for
    -- Command Center below. Both removed rather than branched, because classic is on its way
    -- out and a bars-mode branch would be two texts to keep true instead of one.
    bulletWrapped("The All augments tab lists every augment you own with its effects; search and sort there, and right-click to add to the Augment or Mythical Reroll lists.")
    ...
    ImGui.TextColored(theme.ToVec4(theme.Colors.Muted), "Scripts")
    bulletWrapped("Every AA script in your bags by tier, what each is worth, and a turn-in job you can start and stop.")
    ImGui.TextColored(theme.ToVec4(theme.Colors.Muted), "Chat")
    bulletWrapped("Tabbed chat with its own scrollback and a send box. Item, spell and player links are clickable where the plugin build supports it.")
