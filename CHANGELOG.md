...
### Added
- **The bars — a status strip and a command strip, instead of a hub full of buttons** (opt-in: Settings → General → Dock, or `/itemui dock`). Most of what you open a CoOpt window for is a *question* — is anything running, how full am I, what would selling actually get me — and answering it by opening and closing a window is the tax this removes. **Top bar reports** in eight cells in a fixed order: the CoOpt dot, the session strip (money, augs and mythics still needing a call, scripts collected), bags (+ weight while the game's Inventory window is open), what your rules would sell for right now, the **Loot All / Auto Sell** button pair (each green start becomes its own red **Stop**, in place), the action lane, buffs/songs/aura, and XP/AA/scripts. The lane is the one cell that flexes, and it owns every job's readout in one unchanging spot — idle, looting, a mythical **Take / Pass / Take + reroll** decision that works with the Loot window closed, the finished result (held six seconds), a bags-full problem strip, a sell run, and a script turn-in with the one Stop that lives in the lane. Each slot reserves room for the widest thing it could ever say, so content changing inside a slot never shoves its neighbours around. **Hover popovers** are real panels, not tooltips — you can move into them and click: the buffs one lists what's expiring with a **Recast** that only appears when an item in your bags actually casts that buff (matched by spell id, not name) and it's off cooldown; the sell one groups your items by *the rule that decided them*, which is where players learn their own rules, and only offers **Sell** when you're actually at a merchant. Middle-click pins a popover, Esc closes it, and none of them open while a mythical decision is on screen — a panel that can be clicked also absorbs clicks. **Bottom bar takes commands**: chat owns the left edge, then a launcher row of window chips (an open window is lit; the row folds itself away when chat needs the room — the top bar's CoOpt cell catches everything it drops), then **Native UI** and **Layouts** menus and a **Settings** toggle on the right (the chat console itself lives in its own window — see below). **The Command Center window is retired** — everything it did, including ScriptTracker and the native panel, is on the bar, so it can stay closed for good; `/itemui center` now just makes sure the bar is on screen. Nothing lives on both a bar and inside a window — one home per control. In bars mode the hub drops its twelve-button row and keeps everything else. `UIMode` is a runtime switch, not a one-way door: uncheck it (or `/itemui dock off`) and you are back to today's UI immediately. Player guide: `docs/DOCK_UI.md`.
- **Chat is a real console window, not four lines** — the bottom bar's chat strip is down to a launcher: hidden, or one collapsed line (click it, or its unread badges, to open the window). Everything else moved into a new **Chat window**, a normal companion with its own zone and size: five tabs (All / Main / MQ / Other / CoOpt) with unread counts that clear as you view them, full scrollback instead of the strip's old four-line limit, and **clickable item/spell/player/achievement links** — the raw `\x12` tag soup you used to get pasting a link into the strip is gone. Built on MacroQuest's Zep console widget where the plugin build has one; falls back to a plain list with links stripped to clean text where it isn't. **You can type from it** — the input row runs `/commands` as typed and sends bare text as `/say`, Enter or **Send** to submit — though EverQuest doesn't see your keystrokes while that field has focus, the same tradeoff every other focusable CoOpt window makes.
- **Windows place themselves — zones, magnets, and named layouts** (bars mode). Opening a window no longer drops it wherever it last was: each companion declares a screen zone around the hub (equipment left, bank and item display right, effects below, and so on) and opening one takes the first free slot in its zone, clamped to the screen with the bars' strips subtracted — nothing opens off-screen or on top of another window, on any resolution. Drag a window and it becomes *yours*: it stops auto-placing until you press **Re-tidy** (bottom-bar Layouts menu, Settings, or `/itemui retidy`). Drop a window within 12px of the hub or another window and it snaps flush and *stays attached* — drag the hub and its satellites travel with it; close a window in a snapped column and the ones below slide up; hold **Alt** to drag free of the magnets. **Layout presets** name a whole arrangement — which windows are open, in which zone, at what size. Five ship out of the box (Bag session, Farming, Merchant run, Gearing up, Raid — minimal), all zone-driven so they fit any screen; **Save current as…** captures your exact arrangement, and switching closes what isn't in the preset (pinned windows stay). Presets live in their own `itemui_presets.ini`, so **Revert to Default Layout never deletes them**. Apply from the bottom-bar **Layouts** menu, Settings → General → Layouts, or `/itemui layout <name>`.
- **First run is two questions, not thirteen steps** — how careful CoOpt should be with your stuff (Cautious / Balanced / Aggressive, each a strictly-additive protection profile — picking a looser one later never deletes rules you added), and how much screen to use (Two bars + hub / Top bar only / Windows only). A live strip shows what the current rules would sell right now, in actual platinum. After that **the bar teaches itself**: five hints, each appearing once at its own first real moment — first merchant, first loot run, first mythical decision, first full bag, first rule edit — dismissed forever with one click, replayable with `/itemui hints`. The wizard at `/itemui setup --full` is now four steps and one job: shape the window sizes and columns you will live in, with the live window in front of you. Truly new installs start in bars mode; every upgrade keeps whatever mode it had, and re-running setup can no longer silently flip an existing install's answer.
- **The status bar's cells are membership-only** — Settings → General → Dock turns individual cells on and off; the left-to-right order is the bar's own and never changes, so there is nothing to reorder and no two installs disagree about where a cell lives. (The membership is the `DockSegments=` CSV in `itemui_layout.ini`; the launcher ROW on the command bar is what reorders, with ^ / v arrows in the same Settings section.)
- **When something's broken, the bar says so** — one dismissible strip for the four conditions that used to surface as a console line or nothing: `sell.mac` missing (macro mode only), no sell rules yet (with what the *default* pipeline will actually do — sell unmatched tradeables above the value floor, which is the opposite of safe), bank shown from a days-old snapshot, and running without the plugin. Each says what, what it costs you right now, and offers a fix that exists. And in the Sell view, **the status cell is a link**: click "Will sell" to see *which rule* decided that, with one-click **Keep this item** / **Protect \<type\>** fixes.
- **The native windows got the same honesty pass** (with the plugin loaded; the skin's static labels stand without it): the Command Center's launcher buttons stay lit while their window is open so the panel doubles as a window list, its run buttons say what a click does *now* ("Looting… stop" actually stops), and its status line carries live progress. The merchant strip states the offer before you click ("2,412p | 8 protected by your rules"), turns into **Stop** with progress while a macro sell runs, admits the Lua batch has no stop, and — for the first time — reports failures when it's done ("Sold 31 for 9,884p | 2 failed"). The AA window's Import tells you which file, how many ranks, and what you have, before you confirm.
- **Item Display leads with a verdict** — opening an item now shows the answer before the encyclopedia: an icon/name/subtitle header (theme-colored by usability, folding in the old standalone "Can I use?" banner), a bordered **verdict box** comparing the item against whatever's currently equipped in its slot ("Upgrade over your Ear 2" / "+412 HP +18 AC −6 Mana", green for an upgrade, red for a downgrade, muted for a sidegrade or when there's nothing to compare against yet), and a 6-tile stat grid (HP, AC, Mana, Haste, Attack, Augs filled/total) showing each stat's value alongside its signed, colored delta. The equipped comparison reads live game state (not a stale cache), so it works even if you've never opened the Equipment Companion this session. The full existing stat/effects breakdown is unchanged and un-trimmed — it's one click away under "All stats & effects" (open by default, so nothing you used to see by default is now hidden), and a new **Rules** section surfaces the sell/keep reason plus one-click Keep, Junk, Add to Reroll, and Aug Utility (inventory items only — bank/corpse tabs show the reason without actions that need a bag slot to act on).
- **The EMU bundle is now the complete environment** — Build-Smart compiles the community plugin ecosystem in-tree, pinned and built against the same MacroQuest family as everything else: MQ2MoveUtils, MQ2Cast, MQ2AdvPath, MQ2Twist, MQ2Exchange, MQ2Debuffs, MQ2GroupInfo, MQ2LinkDB, MQ2NetBots (25 plugins total, all enabled in the shipped MacroQuest.ini), plus the standalone prebuilts (eqbcs.exe, MeshGenerator, D3DX9) and the E3 seed configs. E3's `/stick`, `/casting`, and AdvPath follow all work out of the box. MQ2Nav is deferred to its own porting pass (standalone VS solution with vendored dependencies); the crew's proven config runs follow via MQ2AdvPath.

### Changed
- **The layout INI loader is no longer written twice** — `loadLayoutConfig` had two hand-maintained bodies, one for the cache hit and one for the file re-parse, byte-identical for ~120 lines. That duplication has already cost one bug: Equipment geometry went missing from the file branch, so any re-parse dropped the saved window position (fixed previously; the comment at the old `layout.lua:604` records it). Both paths now share one `applyLayoutSection`, so a new key can only be added in one place — no behaviour change, and the eleven Dock keys did not have to be added twice by hand.

### Fixed
- **Fresh installs are CoOpt's own bundle again — with the plugin enabled** — the patcher downloads the (now complete) CoOptUI-EMU zip as the base, so MQ2CoOptUI is safe and ON for fresh installs. The stock E3NextAndMQNextBinary bundle is only the fallback when our release download fails — and on that foreign MQ family the installer force-disables MQ2CoOptUI (loading our plugin into a different MQ build corrupts its Lua runtime: crash in mq2lua invoking a Lua-bound command, hard freeze on `/lua stop`) and CoOpt runs fully in Lua/TLO mode. Fresh installs also survive Windows' 260-character path limit during extraction and show progress on size-less GitHub downloads.
