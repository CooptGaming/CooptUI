...
### Added
- **The bars — a status strip and a command strip, instead of a hub full of buttons** (opt-in: Settings → General → Dock, or `/itemui dock`). Most of what you open a CoOpt window for is a *question* — is anything running, how full am I, what would selling actually get me — and answering it by opening and closing a window is the tax this removes. **Top bar reports** in seven fixed-width slots: plugin state, bags (+ weight while the game's Inventory window is open), what your rules would sell for right now, live loot progress, buffs/songs/aura, XP/AA/scripts, and a session total. The loot slot has five moods in one unchanging width — idle, looting with **Stop**, a mythical **Take/Pass** decision that works with the Loot window closed, the finished result, and a bags-full problem strip. Each slot reserves room for the widest thing it could ever say, so content changing inside a slot never shoves its neighbours around. **Hover popovers** are real panels, not tooltips — you can move into them and click: the buffs one lists what's expiring with a **Recast** that only appears when an item in your bags actually casts that buff (matched by spell id, not name) and it's off cooldown; the sell one groups your items by *the rule that decided them*, which is where players learn their own rules, and only offers **Sell** when you're actually at a merchant. Middle-click pins a popover, Esc closes it, and none of them open while a mythical decision is on screen — a panel that can be clicked also absorbs clicks. **Bottom bar takes commands**: four hover menus (Items / Character / Actions / Game windows) driven by the registry, so an open window is lit and a companion you disabled isn't listed, plus the game's own windows, Settings, and a chat launcher with per-channel unread counts (the chat console itself lives in its own window — see below). **The Command Center window is retired** — everything it did, including ScriptTracker and the native panel, is on the bar, so it can stay closed for good; `/itemui center` now just makes sure the bar is on screen. Nothing lives on both a bar and inside a window — one home per control. In bars mode the hub drops its twelve-button row and keeps everything else. `UIMode` is a runtime switch, not a one-way door: uncheck it (or `/itemui dock off`) and you are back to today's UI immediately. Player guide: `docs/DOCK_UI.md`.
