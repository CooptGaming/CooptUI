...
- **Equipped-item inspect redirects to the CoOpt Item Display** — This server's custom item data garbles the stock inspect window's layout. Right-clicking a worn slot in the game's own Inventory window now opens the CoOpt Item Display for that slot (detected via the hover pipeline — no plugin TLO needed) and squashes the native window that pops. Settings → General toggle, on by default. Bag items stay with the CoOpt Inventory Companion, which already handles them.
