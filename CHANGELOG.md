...
### Fixed
- **The CoOpt skin now ships and installs itself** — no release channel (patcher manifest, CoOpt UI zip, EMU deploy) actually included `uifiles\coopt` or the `coopt_launcher` script, so every native-window feature would have arrived dead for patched users. All three channels now carry them, and because EQ loads skins from its **own** uifiles folder (not MacroQuest's), itemui now syncs the skin into the EQ client at startup whenever files are missing or outdated — it also deletes retired skin files (the trialed loot-window strip) so stale overrides with dead buttons can't linger. itemui also no longer tries to start `coopt_launcher` when the script isn't installed.
