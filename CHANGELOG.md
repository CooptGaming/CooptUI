...
- **Plugin window API (MQ2CoOptUI)** — the plugin now drives native windows directly: `setChecked` (instant, safe un-latch of strip buttons — no more synthetic clicks, timing delays, or mouse-capture risk), `setText` (status lines on any label, not just EditBoxes), `click` (dispatches a real click notification to a native button's handler), and `getMouseOverSlot` (the bag/bank hover above). The Lua side auto-detects the capability and falls back to the old paths on older plugin builds.
...
- **Lock a window open** — Every companion window (Clickies, Bank, Augments, Mythicals, Reroll, AA, Equipment, Item Display, Augment Utility, Command Center, Loot) now has a **Lock** checkbox at its top right. Locked windows are skipped by ESC's close-most-recent behavior, so the panels you play with stay up; deliberate close-alls (Shift+Q, `/itemui hide`, the hub's X) and the window's own X still close them. Locks persist per character in the layout.
