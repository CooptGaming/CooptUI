...
### Added
- **Native merchant strip** — With the new CoOpt UI skin loaded (`/loadskin coopt`), the game's **own Merchant window** gains an Auto Sell button, a Preview (dry run) button, Keep/Junk toggles for the selected item, and a live status line (sell/keep/protected counts, macro progress) — all driven by CoOpt UI through MacroQuest. Routine vendoring no longer needs the CoOpt window open. Toggle under Settings → General ("Native merchant strip"); does nothing unless the skin is loaded. The skin ships in `uifiles/coopt` (only overrides the merchant window — everything else stays your normal UI).
