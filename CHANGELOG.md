...
- **Native inventory hover tooltips** — With the game's own Inventory window open, hovering a worn equipment slot shows the full CoOpt stats tooltip (same renderer as the Equipment Companion). Settings → General toggle, on by default. Bag/bank slot support needs plugin help and comes later.
...
- **Aug Utility button on the native Item Display** — Inspecting an item (right-click) now shows a CoOpt "Aug Utility" button in the corner: one click opens the Augment Utility targeted at that exact item (needs the item in your bags and the MQ2ItemDisplay plugin for the inspected-item TLO).
- **Native item inspect redirects to the CoOpt Item Display** — This server's custom item data garbles the stock inspect window's layout, so with the CoOpt skin loaded, right-click inspecting a bag item now instantly closes the native window and opens the CoOpt Item Display for it. Settings → General toggle, on by default; item links and worn inspects (where CoOpt has no data) keep the native window.
...
- **The right-clicked corpse is no longer skipped by the loot run** — opening a corpse flags it "looted", and the macro's `/hidecorpse looted` hid it from targeting, so the handoff's first corpse was left behind. The native strip now targets the corpse before closing it, and loot.mac honors a pre-targeted corpse on its first pass (one-shot, so skipped-items corpses can't loop).
