...
### Fixed
- **The layout INI loader stopped being written twice** — `loadLayoutConfig` had two hand-maintained bodies, one for the cache hit and one for the file re-parse, byte-identical for ~120 lines. They had already drifted once: Equipment geometry was missing from the file branch, so any re-parse dropped the saved window position and the window's own move-save then wrote defaults back over it. Both paths now share one `applyLayoutSection`, so a new key can only be added in one place.
- **This run's skip count is this run's** — the loot recap's "N skipped" read `skipHistory`, a capped ring buffer that accumulates across runs and is reloaded from disk, so it reported a lifetime total as if it were the current run. The per-run figure is now recorded separately.
