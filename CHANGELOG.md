...
- **AA Browser only lists abilities your character can actually see** — the full id-space scan surfaced every AA in the game's table, including other classes' lines (e.g. Rebirth for classes you don't have active) and unavailable Special entries (the "breath" line). With the updated plugin, the scan now asks the client's own filter (`CanSeeAbility` — the exact check the native AA window uses, which is also where this server's multi-class rules apply), so the browser matches what the game itself would show. Bonus: the filtered scan finishes near-instantly. Without the plugin it falls back to the old full scan.
